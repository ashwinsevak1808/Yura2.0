...
import { LayoutDashboard, ShoppingBag, Package, Users, LogOut, MessageSquare, BarChart3 } from "lucide-react";
...
export default function AdminSidebarWrapper({ children }: { children: React.ReactNode }) {
    ...
    // List of routes that don't require authentication or sidebar
    ...
    useEffect(() => {
        async function checkAuth() {
            ...
            if (!user) {
                ...
                const isLegacyAdmin = user.email === 'ashwinsevak2091@gmail.com'; // Keep temporary fallback
    ...
    // Don't show sidebar on public admin pages
    if (publicAdminRoutes.includes(pathname)) {
        return <>{children}</>;
    }
    ...
    const navItems = [
        ...
        // { name: "Accounting", href: "/admin/accounting", icon: BarChart3 }, // Removed
        { name: "Users", href: "/admin/users", icon: Users },
        { name: "Charges", href: "/admin/charges", icon: BarChart3 }, // Reusing BarChart for "Charges/Economics"
        ...
        <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
            ...
            <aside className="w-72 bg-[#0a0a0a] text-white flex flex-col flex-shrink-0 z-40 relative shadow-2xl">
                ...
                <div className="h-24 flex items-center justify-center border-b border-white/10">
                    <img src="/logo.svg" alt="YURAA" className="h-12 w-auto brightness-0 invert" />
                </div>
